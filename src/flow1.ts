...
// Set ô "Ngày khám" TRONG dialog Khám chuyên khoa = ngay. Ưu tiên ô .input-date (gõ thẳng),
// fallback antd picker (chọn trên lịch, scope dropdown ĐANG MỞ).
async function setNgayKhamCK(page: Page, dialog: Locator, ngay: string): Promise<void> {
  const [dd, mm, yyyy] = ngay.split('/').map((s) => s.trim());
  const iso = `${yyyy}-${mm.padStart(2, '0')}-${dd.padStart(2, '0')}`;

  const inp = dialog.locator('input.input-date').first();
  if (await inp.count()) {
    await inp.scrollIntoViewIfNeeded();
    await inp.click();
    await inp.press('Control+a');
    await inp.press('Delete');
    await inp.pressSequentially(ngay, { delay: 25 });
    await inp.press('Tab');
    await page.waitForTimeout(400);
    if (((await inp.inputValue().catch(() => '')) || '').includes(ngay)) return;
  }
  // antd picker readonly -> mở lịch, chọn ô ngày (chỉ trong dropdown đang mở)
  const picker = dialog.locator('.ant-picker').first();
  await picker.click();
  const ddw = page.locator('.ant-picker-dropdown:not(.ant-picker-dropdown-hidden)').last();
  await ddw.locator('.ant-picker-cell-in-view').first().waitFor({ state: 'visible', timeout: 8000 });
  for (let i = 0; i < 60; i++) {
    const head = (await ddw.locator('.ant-picker-header-view').first().textContent().catch(() => '')) || '';
    const my = /([A-Za-z]{3,})\D*(\d{4})/.exec(head);
    if (!my) break;
    const curM = THANG_EN.findIndex((x) => my[1].startsWith(x));
    ...
    await ddw.locator(back ? '.ant-picker-header-prev-btn' : '.ant-picker-header-next-btn').first().click();
    await page.waitForTimeout(250);
  }
  await ddw.locator(`.ant-picker-cell-in-view[title="${iso}"]`).first().click();
  await page.waitForTimeout(400);
}

// Mở dialog "Khám chuyên khoa" trên editor tờ điều trị, (sửa ngày khám), chọn khoa + nội dung, Xác nhận.
export async function moKhamChuyenKhoa(page: Page, maKhoa: string, noiDung: string, ngayKham?: string): Promise<void> {
  ...
  if (ngayKham) {
    await step(page, `Ngày khám chuyên khoa = ${ngayKham}`, async () => {
      await setNgayKhamCK(page, dialog, ngayKham);
    }, { retries: 1 });
  }
